# SecureScan Remediation

**Severity:** HIGH
**Tool:** trufflehog
**File:** `/tmp/scans/76ced04c-62a9-4b77-bfa6-e76486158d97/artifacts/cert/server.key`
**OWASP Category:** A04

## Description

PrivateKey

## Proposed Fix

Ne stockez jamais de secrets dans le code ou le dépôt. Utilisez des variables d'environnement, un gestionnaire de secrets (Vault, AWS Secrets Manager, etc.) et limitez la portée des clés.

## Raw Code

```
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----

```
